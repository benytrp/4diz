  ...
  <script>
    ...
    class Consciousness4DPaintStudio{
      ...
      setupProjectionControls(){
        ...
        sliders.forEach(key=>{
          ...
          slider.addEventListener('dblclick',(e)=>{
            e.preventDefault();
            const current=param.value; const raw=prompt(`Set ${key} (current: ${current.toFixed(3)}, safe range: ${param.min}-${param.max})`);
            const v=parseFloat(raw); if(!isNaN(v)){ param.value=v; slider.value=Math.max(slider.min,Math.min(slider.max,v)); this.applyValidationStyles(); this.update3DProjections(); }
          });
        });
      ...
      onResize(){
        ...
        if (w === this.renderer.domElement.width && h === this.renderer.domElement.height) return;
          ...
          try{
            ...
            if (obj.projection?.params){
              Object.entries(obj.projection.params).forEach(([k,v])=>{
                if (projParams[k] && typeof v === 'number'){ projParams[k].value=v; const slider=document.getElementById(`p_${k}`); if(slider){ slider.value=Math.max(slider.min,Math.min(slider.max,v)); } }
              });
              this.applyValidationStyles();
            }
      ...
      animate(){
        ...
        document.getElementById('fps').textContent = this.hud.smoothed.toFixed(0);
      }
    }

    // Boot
    window.addEventListener('DOMContentLoaded', ()=>{ new Consciousness4DPaintStudio(); });
